...
Widget buildIndicator(BuildContext context, IndicatorStatus status,
    LoadingMoreBase listRepository) {
  ...
  switch (status) {
    ...
    case IndicatorStatus.empty:
      widget = Text(
        "什么也没有找到(つд⊂)",
        style: TextStyle(color: Colors.grey, fontSize: ScreenUtil().setSp(46)),
      );
      widget = _setbackground(context, true, widget, double.infinity);
      widget = GestureDetector(
        onTap: () {
...
Widget _setbackground(
    BuildContext context, bool full, Widget widget, double height) {
  widget = Container(
