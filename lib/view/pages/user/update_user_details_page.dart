...
import 'package:flutter_application/view/pages/change_avatar.dart';
...
class _UpdateUserDetailPageState extends State<UpdateUserDetailPage> {
                ...
                trailing: Container(
                  ...
                  child: CircleAvatar(
                    backgroundImage: model.user.avatarUrl == null
                        ...
                        : NetworkImage(
                            NetConfig.ip + '/images/' + model.user.avatarUrl),
                  ),
                ),
              ),
              MyListTile(
                ...
                onTap: () {
                  Navigator.push(
                      context,
                      CupertinoPageRoute(
                          builder: (context) => ChangeAvatar(type: 2)));
                },
                leading: Text('个人主页背景',
                    style: TextStyle(fontSize: ScreenUtil().setSp(44))),
                trailing: Container(
  ...
  showPickerGender(BuildContext context, UserModel model) {
    Picker(
        ...
        textStyle:
            TextStyle(color: Colors.grey, fontSize: ScreenUtil().setSp(48)),
        onConfirm: (Picker picker, List value) async {
          print(value.first);
          var gender = value.first;
          var res = await NetRequester.request(
              Apis.updateUserProperty('gender', gender));
          if (res['code'] == '1') {
  ...
  showPickerCity(BuildContext context, UserModel model) {
    ...
    Picker(
        ...
        onConfirm: (Picker picker, List value) async {
          ...
          print(city);
          var res =
              await NetRequester.request(Apis.updateUserProperty('city', city));
          if (res['code'] == '1') {
  ...
  showPickerDate(BuildContext context, UserModel model) {
    ...
    Picker(
        ...
        onConfirm: (Picker picker, List value) async {
          ...
          var res = await NetRequester.request(
              Apis.updateUserProperty('birthDay', birthDay));
          if (res['code'] == '1') {
