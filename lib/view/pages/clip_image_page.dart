...
import 'package:flutter_application/utils/clip_editor_helper.dart';
...
class _ClipImgPageState extends State<ClipImgPage> {
        ...
        initEditorConfigHandler: (state) {
          return EditorConfig(
              editorMaskColorHandler: (context, pointerDown) =>
                  Colors.black.withOpacity(pointerDown ? 0.4 : 0.8),
  ...
  _upLoadImg() async {
    ...
    if (res == 0) {
      ...
      var result =
          await NetRequester.request('/user/updateUserProperty', data: map);
      if (result['code'] == '1') {
        Toast.popToast('上传成功');
